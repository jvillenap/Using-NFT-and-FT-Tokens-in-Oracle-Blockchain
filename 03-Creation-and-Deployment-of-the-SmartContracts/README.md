...
8. At this point we should create the implementation of these custom methods. For simplicity we provide in the file [NFT Custom Code]()[NFT_CustomMethods.ts](./src/NFT_CustomMethods.ts).  all the implementation for such methods, so we only need to substitute the signatures autogenerated by AppBuilder, by the content on the referenced file.
